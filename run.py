...
def process_file(file_name, result):
    ...
    with open(file_name, "r") as readFile:
        ...
        source_data = extract_values(data, 'name,location')
        # result = {}
            ...
            if b.startswith('Segment-'):
                ...
                if not (source_data[x + 1].startswith(segment) or (source_data[x + 2].startswith('Segment-'))):
                    if source_data[x + 2].find('/') != -1:
                        # print(element_path + '/' + b + ':' + source_data[x + 2])
...
for r, d, f in os.walk(path):
    for file in f:
        ...
        a = file_path[start_src_index + 1:start_dest_index]
        c = file_path[start_dest_index+4:end_index+16]
        if a+':'+c in versions:
            versions[a + ':' + c] = process_file(os.path.join(r, file), versions[a + ':' + c])
        else:
            versions[a+':'+c] = process_file(os.path.join(r, file), {})
